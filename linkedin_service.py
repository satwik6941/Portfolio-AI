...
class LinkedInJobAPI:
    ...
    def __init__(self):
        ...
        try:
            ...
            self.ai_data_service = AIDataService()
            self.logger.info("✅ AI data service initialized for LinkedIn fallback")
        except Exception as e:
            self.logger.warning(f"⚠️ Could not initialize AI data service: {e}")
            self.ai_data_service = None
    ...
    def _fallback_job_search(self, keywords: str, location: str, limit: int) -> List[Dict]:
        """Enhanced AI-powered fallback job search when LinkedIn API is not available"""
        self.logger.info("Using AI-enhanced fallback job search method")
        
        try:
            # Try to use AI data service first
            if self.ai_data_service:
                ...
                if ai_jobs and len(ai_jobs) > 0:
                    # Mark as LinkedIn source and enhance
                    for job in ai_jobs:
                        job['source'] = 'linkedin_ai_fallback'
                        job['linkedin_url'] = f"https://www.linkedin.com/jobs/view/{job.get('id', 'ai_generated')}"
                        job['application_url'] = f"https://careers.{job.get('company', 'company').lower().replace(' ', '')}.com/apply"
                    
                    self.logger.info(f"✅ Generated {len(ai_jobs)} AI-powered LinkedIn job listings")
                    return ai_jobs
            
            # Fallback to minimal hardcoded data if AI fails
            self.logger.warning("AI data service not available, using minimal fallback")
            return self._minimal_linkedin_fallback(keywords, location, limit)
            
        except Exception as e:
            self.logger.error(f"AI fallback failed: {e}, using minimal fallback")
            return self._minimal_linkedin_fallback(keywords, location, limit)
    
    def _minimal_linkedin_fallback(self, keywords: str, location: str, limit: int) -> List[Dict]:
        ...
        mock_jobs = [
            {
                ...
                'source': 'linkedin_minimal_fallback'
            }
    ...
    def get_trending_skills(self, industry: str = "") -> List[str]:
        """Get trending skills from LinkedIn using AI-enhanced insights"""
        # This would require LinkedIn Skills API access
        # For now, use AI-enhanced industry-relevant trending skills
        
        try:
            # Try to use AI data service for trending skills
            if self.ai_data_service:
                ai_skills = self.ai_data_service.generate_trending_skills(industry)
                if ai_skills and len(ai_skills) > 0:
                    self.logger.info(f"✅ Generated {len(ai_skills)} AI trending skills for {industry}")
                    return ai_skills[:12]  # Return top 12 skills
        
        except Exception as e:
            self.logger.warning(f"AI trending skills failed: {e}, using static fallback")
        
        # Fallback to static skill trends
        skill_trends = {
            'technology': [
    ...
    def get_salary_insights(self, job_title: str, location: str = "") -> Dict:
        """Get salary insights for a job title and location using AI-enhanced data"""
        # This would use LinkedIn Salary Insights API
        # For now, use AI-enhanced fallback with market data
        
        try:
            # Try to use AI data service for salary insights
            if self.ai_data_service:
                ai_salary_data = self.ai_data_service.generate_salary_insights(job_title, location)
                if ai_salary_data:
                    self.logger.info(f"✅ Generated AI salary insights for {job_title} in {location}")
                    return ai_salary_data
        
        except Exception as e:
            self.logger.warning(f"AI salary insights failed: {e}, using static fallback")
        
        # Fallback to static salary data
        base_salaries = {
